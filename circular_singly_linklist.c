...
void insertatlast(int data)
{
    ...
    new_node->info = data;
    cur = head;
    for (int i = 1; i < current_total_nodes; i++)
    {
        cur = cur->next;
    }
    new_node->next = head;
...
{
    cur = head;
    for (int i = 1; (i < current_total_nodes - 1); i++)
    {
        cur = cur->next;
    }
    cur1 = cur->next;
...
int main()
{
    ...
    while (choice != 9)
    {
        printf("Enter command 1 to create a  circular linklist\n ");
        ...
        case 5:
            ...
            break;
        case 6:
            printf("You are inserting an element at last :\n");
            printf("enter the data for the node :\n");
            scanf("%d", &data);
            insertatlast(data);
            break;
